...
public class StudentUtils {
    ...
    {
        Collections.sort(list, (s1, s2) -> Double.compare(s1.average(), s2.average()));
    }
    ...
    {
        return list.stream().mapToDouble(Student::average).average().orElse(0.0);
    }

    public static List<String> goodStudentName(List<Student> list)
    {
        return list.stream().filter(Student::isGoodStudent).map(student -> student.name).collect(Collectors.toList());
    } 
}
